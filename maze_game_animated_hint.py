...
def neighbors_open(walls, x, y):
    """Return neighbor cells reachable from (x,y) according to walls."""
    res = []
...
def bfs_shortest_path(walls, start, goal):
    ...
    while q:
        x, y = q.popleft()
        for nx, ny in neighbors_open(walls, x, y):
...
def draw_hint_path(screen, path):
    """
    Draw hint as translucent fill + center line.
    """
    if not path:
        return
...
def main():
    ...
    pygame.display.set_caption("Maze Game (Animated Generator + BFS Hint)")
    ...
    def recalc_hint():
        ...
        if show_hint and not generating:
            hint_path = bfs_shortest_path(walls, (player_x, player_y), (goal_x, goal_y))
        else:
            hint_path = []

    def start_generation(animated=True):
        nonlocal generating, gen, walls, visited, player_x, player_y, won, moves
        nonlocal current_cell, carved_edge, show_hint, hint_path
    ...
    while running:
        ...
        if generating:
            info = f"Generating... speed: {gen_speed} steps/s | G: anim | R: instant | +/-: speed"
        else:
            hint_txt = "ON" if show_hint else "OFF"
            info = f"Moves: {moves} | H: hint({hint_txt}) | Arrows: move | SPACE: restart | G: anim | R: instant"
