...
def animated_dfs_generator(cols, rows, seed=None):
    ...
    Yields dict:
      walls, visited, current=(x,y), stack=[...], carved=((cx,cy),(nx,ny)) or None
    """
    if seed is not None:
        random.seed(seed)
    ...
    while stack:
        ...
        if neighbors:
            ...
            # carve walls between (cx,cy) and (nx,ny)
            ...
            stack.pop()
            # backtrack step (optional yield for animation feeling)
            yield {
                "walls": walls,
                "visited": visited,
                "current": (cx, cy),
...
def main():
    ...
    pygame.display.set_caption("Maze Game (Animated Generator + Runner)")
    ...
    generating = True
    gen_speed = 220  # steps per second (change with +/-)
    gen_accum = 0.0
    gen = None
    current_cell = (0, 0)
    carved_edge = None  # ((cx,cy),(nx,ny))

    def start_generation(animated=True):
        nonlocal generating, gen, walls, visited, player_x, player_y, won, moves, current_cell, carved_edge
        player_x, player_y = 0, 0
        won = False
        moves = 0
        carved_edge = None
        ...
        if animated:
            ...
            # fast generation (no animation)
            generating = False
            # do full run of generator
    ...
    # start with animated generation
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_g:
                    ...
                    start_generation(animated=False)  # instant
        ...
        # -----------------------------
        if generating:
            # run N steps based on gen_speed
        ...
        # visited highlight (during generation)
        if generating:
            for y in range(ROWS):
                for x in range(COLS):
                    if visited[idx(x, y)]:
        ...
        # goal + player (only when not generating)
        if not generating:
        ...
        if generating:
            info = f"Generating...  speed: {gen_speed} steps/s | G: regen anim | R: instant | +/-: speed"
        else:
            info = f"Moves: {moves} | Arrows: move | SPACE: restart | G: regen anim | R: instant"
